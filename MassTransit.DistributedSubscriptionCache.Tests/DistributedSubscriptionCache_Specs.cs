namespace MassTransit.DistributedSubscriptionCacheX.Tests
{
	...
	public class When_using_a_distributed_subscription_cache
	{
		...
		[Test]
		public void Removing_a_subscription_should_mean_it_no_longer_exists_middle()
		{
			DistributedSubscriptionCache cacheA = new DistributedSubscriptionCache();
			DistributedSubscriptionCache cacheB = new DistributedSubscriptionCache();
			...
			string urlC = "http://localhost/feature.html";
			Subscription sampleC = new Subscription(_name, new Uri(urlC));
			cacheB.Add(sampleC);
			...
			Assert.That(subscriptions[2].EndpointUri.ToString(), Is.EqualTo(urlC));
			...
			Assert.That(subscriptions[0].EndpointUri.ToString(), Is.EqualTo(urlA));
			Assert.That(subscriptions[1].MessageName, Is.EqualTo(_name));
			Assert.That(subscriptions[1].EndpointUri.ToString(), Is.EqualTo(urlC));
		}
