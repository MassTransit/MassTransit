namespace MassTransit.EntityFrameworkCoreIntegration.Tests.TransactionConfiguration
{
        ...
        where T : ITestDbParameters, new()
    {
        ...
        [Test]
        public void CreatePessimistic_Should_Accept_Transactions_Disabled()
        {
            var repository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
        ...
        [Test]
        public void CreatePessimistic_Should_Accept_Transactions_Enabled_Explicitly()
        {
            var repository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
        ...
        [Test]
        public void CreatePessimistic_Should_Work_With_DbContextFactory_And_Transaction_Setting()
        {
            // Using delegate factory instead of ISagaDbContextFactory for simplicity
            var repository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
        ...
        [Test]
        public void CreatePessimistic_Should_Work_With_Query_Customization_And_Transaction_Setting()
        {
            Func<IQueryable<SimpleSaga>, IQueryable<SimpleSaga>> customization = q => q.Where(s => s.Name != null);

            var repository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
                RawSqlLockStatements, customization, false);
        ...
        [Test]
        public void CreatePessimistic_Should_Work_Without_Lock_Statement_Provider()
        {
            var repository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
                null, null, false);
            ...
            Assert.That(repository, Is.Not.Null,
                "Repository should be created successfully without lock statement provider");
        }

        [Test]
        public void Should_Create_Different_Repository_Types_With_Same_Transaction_Setting()
        {
            var optimisticRepository = EntityFrameworkSagaRepository<SimpleSaga>.CreateOptimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
                null, false);

            var pessimisticRepository = EntityFrameworkSagaRepository<SimpleSaga>.CreatePessimistic(
                () => new SimpleSagaContextFactory().CreateDbContext(DbContextOptionsBuilder),
                RawSqlLockStatements, null, false);

            Assert.That(optimisticRepository, Is.Not.Null);
            Assert.That(pessimisticRepository, Is.Not.Null);
            Assert.That(optimisticRepository, Is.Not.EqualTo(pessimisticRepository),
                "Different repository types should be created");
        }
    }
