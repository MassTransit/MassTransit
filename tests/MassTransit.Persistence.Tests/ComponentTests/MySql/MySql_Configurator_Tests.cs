namespace MassTransit.Persistence.Tests.ComponentTests.MySql
{
    ...
    public class MySql_Configurator_Tests : MySql_Tests
    {
        ...
        public void Configurator_finds_other_parts()
        {
            ...
            configurator.UsingMySql("my connection string", conf => conf.SetOptimisticConcurrency());
            configurator.Register(repositoryServices);

            var provider = services.BuildServiceProvider();
            var instance = provider.GetRequiredService<IMySqlRepositoryConfigurator<InlinedSaga>>();
