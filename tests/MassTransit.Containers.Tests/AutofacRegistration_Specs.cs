namespace MassTransit.Containers.Tests
{
                ...
                builder.Register(context =>
                    {
                        return Bus.Factory.CreateUsingInMemory(x =>
                        {
                            ...
                            {
                                e.UseLifetimeScope(context.Resolve<ILifetimeScope>());
                                e.ConfigureConsumers(context.Resolve<IBusRegistrationContext>());
                            });
                        });
                    })
                    .As<IBus>()
    ...
    [TestFixture]
    public class AutofacContainer_Setup_Using_AddMassTransit :
        AsyncTestFixture
    {
        [Test]
        public async Task Should_work_with_lifecycle_managed_bus()
        ...
        public AutofacContainer_Setup_Using_AddMassTransit()
            : base(new InMemoryTestHarness())
        {
        }
            ...
            protected override void Load(ContainerBuilder builder)
            {
                builder.AddMassTransit(x =>
                {
                    ...
                    x.UsingInMemory((context, cfg) =>
                    {
                        ...
                        {
                            e.UseLifetimeScope(context.GetRequiredService<IComponentContext>().Resolve<ILifetimeScope>());
