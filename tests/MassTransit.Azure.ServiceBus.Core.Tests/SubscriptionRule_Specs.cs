namespace MassTransit.Azure.ServiceBus.Core.Tests
{
        ...
        AsyncTestFixture
    {
        ...
        public async Task Should_update_the_rule_filter_expression()
        {
            ...
            var ruleResults = await managementClient.GetRuleAsync(topicName, subscriptionName,ruleName);
        ...
        public async Task Should_update_the_rule_action_expression()
        {
            ...
            var bus = Bus.Factory.CreateUsingAzureServiceBus(x =>
            {
                ...
                x.ReceiveEndpoint(queueName, e =>
                {
                    e.Subscribe(topicName, subscriptionName, cb =>
                    {
                        var actionRule = new CreateRuleOptions(ruleName, new SqlRuleFilter("0 = 1"))
                        {
                            Action = new SqlRuleAction("SET A = 1")
                        };
            ...
            bus = Bus.Factory.CreateUsingAzureServiceBus(x =>
            {
                ...
                x.ReceiveEndpoint(queueName, e =>
                {
                    e.Subscribe(topicName, subscriptionName, cb =>
                    {
                        var actionRule = new CreateRuleOptions(ruleName, new SqlRuleFilter("1 = 1"))
                        {
                            Action = new SqlRuleAction("SET A = 2")
                        };
