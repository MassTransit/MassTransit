...
A large scope of functionality that users expect from MassTransit solutions are not available to
MassTransit applications built leveraging Azure Functions. We now recommend that developers consider
using the Azure Container App feature set which lets MassTransit work as it always has, while also supporting
