...
This concept comes up often enough that it warrents its own special section. 
...
public interface SubmitOrder : CoreEvent
{
	public string Sku { get; }
	public int Quantity { get; }
}
...
If you try and consume a `Batch<CoreEvent>` and expect to get a variety of types, one of which would be `SubmitOrder`. In OOP land, that makes all of the sense in the world, but in MassTransit Contract design it does not. The application has said that it cares about batches of `CoreEvent` so it will only get back the single property `User`. This is not a symptom of using System.Text.Json, this has been the standard behavior of MassTransit since day 1, even when using Netwonsoft.Json. MassTransit will always respect the contract that has been designed. 
