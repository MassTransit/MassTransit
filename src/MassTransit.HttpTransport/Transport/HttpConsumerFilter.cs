...
namespace MassTransit.HttpTransport.Transport
{
    ...
    {
        static readonly ILogger _logger = Logger.Get<HttpConsumerFilter>();
        ...
        public async Task Send(HttpHostContext context, IPipe<HttpHostContext> next)
        {
            ...
            finally
            {
                ...
                _logger.LogDebug("Consumer {0} received, {1} concurrent", metrics.DeliveryCount, metrics.ConcurrentDeliveryCount);
            }
        }
    }
}
