...
namespace MassTransit.WebJobs.ServiceBusIntegration
{
        ...
        ISendTransportProvider
    {
        ...
        public ServiceBusAttributeSendTransportProvider(IBinder binder, ILogger logger, CancellationToken cancellationToken)
        {
            _binder = binder;
            _logger = logger;
        ...
        public async Task<ISendTransport> GetSendTransport(Uri address)
        {
            ...
            _logger.LogDebug("Creating Send Transport: {0}", queueOrTopicName);

            var sendEndpointContext = new CollectorMessageSendEndpointContext(queueOrTopicName, _logger, collector, _cancellationToken);
