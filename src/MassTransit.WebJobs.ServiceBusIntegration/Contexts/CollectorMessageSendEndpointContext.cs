...
namespace MassTransit.WebJobs.ServiceBusIntegration.Contexts
{
        ...
        SendEndpointContext
    {
        ...
        public CollectorMessageSendEndpointContext(string path, ILogger logger, IAsyncCollector<Message> collector, CancellationToken cancellationToken)
        {
        ...
        {
            _logger.LogDebug("Sending message: {0}", message.MessageId);
        ...
        {
            _logger.LogDebug("Scheduling message: {0} for delivery at {1}", message.MessageId, scheduleEnqueueTimeUtc);
