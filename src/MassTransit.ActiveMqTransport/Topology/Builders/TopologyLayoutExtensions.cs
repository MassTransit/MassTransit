...
namespace MassTransit.ActiveMqTransport.Topology.Builders
{
    ...
    public static class TopologyLayoutExtensions
    {
        ...
        public static void LogResult(this BrokerTopology layout)
        {
            ...
            {
                _logger.LogInformation("Topic: {0}, type: {1}, durable: {2}, auto-delete: {3}", exchange.EntityName, exchange.Durable, exchange.AutoDelete);
            }

            foreach (var consumer in layout.Consumers)
            {
                _logger.LogInformation("Consumer: source {0}, destination: {1}, selector: {2}", consumer.Source.EntityName, consumer.Destination.EntityName,
                    consumer.Selector);
            }
        }
    }
