        ...
        where TArguments : class
    {
        ...
        public async Task Send(ExecuteActivityContext<TArguments> context, IPipe<ExecuteActivityContext<TArguments>> next)
        {
            if (_log.IsDebugEnabled)
                _log.DebugFormat("Host: {0} Executing: {1}", context.ReceiveContext.InputAddress, context.Execution.TrackingNumber);
            ...
            try
            {
                ...
                try
                {
                    ExecutionResult result = await context.Activity.Execute(context.Execution);
                ...
                if (exception != null)
                {
                    ExecutionResult result = context.Execution.Faulted(exception);
