...
namespace MassTransit.Courier.Events
{
    ...
    public class RoutingSlipActivityFaultedEventFilter :
        IRoutingSlipEventFilter
    {
        T IRoutingSlipEventFilter.Filter<T>(T message, RoutingSlipEventContents contents)
        {
            var source = message as RoutingSlipActivityFaulted;
            if (source == null)
                return message;

            RoutingSlipActivityFaulted result = new RoutingSlipActivityFaultedMessage(
                source.Host,
