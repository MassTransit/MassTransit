...
namespace MassTransit.Courier.Events
{
    ...
    public class RoutingSlipCompensationFailedEventFilter :
        IRoutingSlipEventFilter
    {
        T IRoutingSlipEventFilter.Filter<T>(T message, RoutingSlipEventContents contents)
        {
            var source = message as CompensationFailed;
            if (source == null)
                return message;

            var result = new CompensationFailed(
                source.Host,
                source.TrackingNumber,
                source.ActivityName,
                source.ActivityTrackingNumber,
                ((RoutingSlipActivityCompensationFailed)source).Timestamp,
                ((RoutingSlipActivityCompensationFailed)source).Duration,
                ((RoutingSlipCompensationFailed)source).Timestamp,
                ((RoutingSlipCompensationFailed)source).Duration,
