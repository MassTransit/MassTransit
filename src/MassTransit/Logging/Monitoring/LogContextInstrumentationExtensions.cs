namespace MassTransit.Logging
{
    ...
    public static class LogContextInstrumentationExtensions
    {
        ...
        public static StartedInstrument? StartOutboxDeliveryInstrument(this ILogContext logContext, OutboxConsumeContext outboxConsumeContext, OutboxMessageContext context)
        {
            if (!_isConfigured || !_outboxDeliveryTotal.Enabled)
                return null;
            ...
            AddCustomTags(tagList, outboxConsumeContext);
        ...
        static void AddCustomTags(TagList tagList, ReceiveContext receiveContext)
        {
            if (receiveContext.TryGetPayload<IMetricsContext>(out var metricsTagsFeature))
            {
                AddCustomTags(tagList, metricsTagsFeature);
            }
        }

        static void AddCustomTags(TagList tagList, ConsumeContext consumeContext)
        {
            if (consumeContext.TryGetPayload<IMetricsContext>(out var metricsTagsFeature))
            {
                AddCustomTags(tagList, metricsTagsFeature);
            }
        }

        static void AddCustomTags(TagList tagList, SendContext sendContext)
        {
            if (sendContext.TryGetPayload<IMetricsContext>(out var metricsTagsFeature))
            {
                AddCustomTags(tagList, metricsTagsFeature);
            }
        }

        static void AddCustomTags(TagList tagList, IMetricsContext metricsTagsFeature)
        {
            var customTags = metricsTagsFeature.Tags;
