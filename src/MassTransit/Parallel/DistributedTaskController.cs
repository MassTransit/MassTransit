...
namespace MassTransit.Parallel
{
		...
		where TOutput : class
	{
		...
		public DistributedTaskController(IServiceBus bus, IEndpointCache endpointCache, TTask distributedTask)
		{
			_bus = bus;
		...
		private void DispatchSubTaskToWorkers()
		{
			...
			while (workers.Count > 0)
			{
				foreach (Worker worker in workers)
				{
					...
					IEndpoint endpoint = _endpointCache.GetEndpoint(new Uri(worker.Address));
					if (endpoint == null)
						continue;
