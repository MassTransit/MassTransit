...
namespace MassTransit.SubscriptionConnectors
{
	...
	public interface AsyncInstanceSubscriptionConnector :
		InstanceConnector
	{
		Type MessageType { get; }
	}

    public class AsyncInstanceSubscriptionConnector<TConsumer, TMessage> :
		...
		where TMessage : class
	{
		...
		public UnsubscribeAction Connect(IInboundPipelineConfigurator configurator, object instance)
		{
			...
			var sink = new InstanceMessageSink<TMessage>(MultipleHandlerSelector.ForHandler(
				HandlerSelector.ForHandler<TMessage>(message => consumer.Consume(message).Wait())));
