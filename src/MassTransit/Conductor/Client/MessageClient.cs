namespace MassTransit.Conductor.Client
{
        ...
        where TMessage : class
    {
        ...
        public async Task Consume(ConsumeContext<Down<TMessage>> context)
        {
            ...
            {
                _log.DebugFormat("Endpoint Down received: (service-address: {0}, endpoint-address: {1}, client-id: {2})", context.Message.ServiceAddress,
                    context.Message.Endpoint.EndpointAddress, ClientId);
            }
        ...
        {
            if (_log.IsDebugEnabled)
                _log.DebugFormat("Requesting Link to {0} (client-id: {1})", TypeMetadataCache<TMessage>.ShortName, ClientId);
            ...
            using (RequestHandle<Link<TMessage>> request = _clientFactory.CreateRequestClient<Link<TMessage>>().Create(new {ClientId}, cancellationToken))
            {
                ...
                if (_log.IsDebugEnabled)
                    _log.DebugFormat("Linked to {0}", response.Message.Endpoint.InstanceAddress);
            }
        }
