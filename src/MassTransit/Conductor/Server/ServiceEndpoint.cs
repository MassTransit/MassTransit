namespace MassTransit.Conductor.Server
{
        ...
        IServiceEndpoint
    {
        ...
        async Task NotifyUp()
        {
            ...
            catch (Exception exception)
            {
                if (_log.IsWarnEnabled)
                    _log.Warn($"Failed to notify service endpoint up: {EndpointInfo.InstanceAddress}", exception);
            }
        }
        ...
        async Task NotifyDown()
        {
            ...
            {
                if (_log.IsWarnEnabled)
                    _log.Warn($"Failed to notify service endpoint down: {EndpointInfo.InstanceAddress}", exception);
            }
        }
