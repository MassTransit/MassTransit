namespace MassTransit.Serialization
{
    ...
    public class SystemTextJsonConsumeContext : DeserializerConsumeContext
    {
        ...
        public SystemTextJsonConsumeContext(ReceiveContext receiveContext, MessageEnvelope envelope) : base(receiveContext)
        {
        ...
        public override bool TryGetMessage<T>(out ConsumeContext<T> message)
        {
            lock (_locker)
            {
                ...
                if (_supportedTypes.Any(x => typeUrn.Equals(x, StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    var obj = messageToken.ToObject(deserializeType);
        ...
        {
            var messageToken = (JsonElement)message;
            if (messageToken.ValueKind == JsonValueKind.Null)
            {
