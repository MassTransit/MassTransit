...
namespace MassTransit.Transports.Outbox
{
    ...
    {
        private readonly IOutboxSendTransportContext _context;

        public OutboxSendTransport(IOutboxSendTransportContext context)
        {
            _context = context;
        }
        ...
        public async Task Send<T>(T message, IPipe<SendContext<T>> pipe, CancellationToken cancellationToken) where T : class
        {
            ...
            if(!context.TryGetPayload(out IOutboxTransportRepository repository))
            {
                throw new OutboxException("Couldn't resolve outbox repository. Configuration is incorrect.");
            }
            ...
            else if (context.Headers.TryGetHeader("MT-Outbox-Sweeper", out var _))
            {
                await _context.RealSendTransport.Send(message, pipe, cancellationToken).ConfigureAwait(false);
