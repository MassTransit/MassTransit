...
namespace MassTransit.Pipeline.Observables
{
    ...
    public class PublishMessageObserverAdapter<T> :
        IPublishMessageObserver<T>
        where T : class
    {
        readonly IPublishObserver _observer;

        public PublishMessageObserverAdapter(IPublishObserver observer)
        {
            _observer = observer;
        }
        ...
        public Task PublishFault(PublishContext<T> context, Exception exception)
        {
