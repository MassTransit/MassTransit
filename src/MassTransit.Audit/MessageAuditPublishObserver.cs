...
namespace MassTransit.Audit
{
    ...
    public class MessageAuditPublishObserver : IPublishObserver
    {
        readonly MessageAuditStore _store;
        readonly MessageAuditSendObserver.MetadataFactory _metaDataFactory;
        readonly AuditSpecification _specification;

        public MessageAuditPublishObserver(MessageAuditStore store,
            MessageAuditSendObserver.MetadataFactory metaDataFactory,
        ...
        public Task PrePublish<T>(PublishContext<T> context) where T : class => Task.FromResult(0);
