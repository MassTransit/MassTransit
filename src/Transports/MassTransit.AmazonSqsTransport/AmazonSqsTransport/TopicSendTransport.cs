            ...
            where T : class
        {
            ...
            public async Task Send(ClientContext context)
            {
                ...
                try
                {
                    ...
                    var request = await context.CreatePublishRequest(_context.EntityName, sendContext.Body.GetString()).ConfigureAwait(false);

                    _context.SnsSetHeaderAdapter.Set(request.MessageAttributes, sendContext.Headers);

                    _context.SnsSetHeaderAdapter.Set(request.MessageAttributes, MessageHeaders.ContentType, sendContext.ContentType.ToString());
