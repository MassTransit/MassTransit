...
namespace MassTransit.Azure.ServiceBus.Core.Tests
{
    ...
    [TestFixture]
    public class Publishing_message_to_an_endpoint_with_remove_subscriptions :
        AzureServiceBusTestFixture
    {
        Task<ConsumeContext<PingMessage>> _handler;
        readonly TimeSpan _timeout;

        public Publishing_message_to_an_endpoint_with_remove_subscriptions()
        {
        ...
        {
            configurator.ReceiveEndpoint(host, c =>
            {
                c.RemoveSubscriptions = true;
                _handler = HandledByConsumer<PingMessage>(c);
            });
        }
        ...
        public async Task Should_succeed()
        {
            ...
            Assert.AreEqual(await Task.WhenAny(_handler, Task.Delay(_timeout)), _handler);
        }
    }
}
