        ...
        where TSaga : class, ISaga
    {
            ...
            try
            {
                using (var transaction = await dbContext.Database.BeginTransactionAsync(_isolationLevel, context.CancellationToken).ConfigureAwait(false))
                {
                    ...
                    try
                    {
                        if (instance == null)
                        {
                            ...
                            if (rowLockQuery != null)
                                ...
                                instance = await queryable.SingleOrDefaultAsync(x => x.CorrelationId == sagaId, context.CancellationToken)
                                    .ConfigureAwait(false);
                        }
                    ...
                    catch (DbUpdateConcurrencyException)
                    {
                        ...
                        {
                            _logger.LogWarning("The transaction rollback failed", innerException);
                        }
        ...
        static async Task<bool> PreInsertSagaInstance<T>(DbContext dbContext, TSaga instance, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogDebug("SAGA:{0}:{1} Dupe {2} - {3}", TypeMetadataCache<TSaga>.ShortName, instance.CorrelationId,
