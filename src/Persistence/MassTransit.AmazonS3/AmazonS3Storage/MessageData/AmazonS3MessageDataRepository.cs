namespace MassTransit.AmazonS3Storage.MessageData
{
    using System;
        ...
        IBusObserver
    {
        ...
        public AmazonS3MessageDataRepository(string bucket) : this(new AmazonS3Client(), bucket)
        {
        }

        public AmazonS3MessageDataRepository(AmazonS3Config config, string bucket) : this(new AmazonS3Client(config), bucket)
        {
        }
                ...
                if (MessageDataDefaults.TimeToLive != null && MessageDataDefaults.TimeToLive.Value.Days > 0)
                {
                    await _s3Client.PutLifecycleConfigurationAsync(new PutLifecycleConfigurationRequest
                    {
                        ...
                        Configuration = new LifecycleConfiguration
                        {
                            ...
                            {
                                new LifecycleRule { Id = RuleName, Expiration = new LifecycleRuleExpiration { Days = MessageDataDefaults.TimeToLive.Value.Days } }
                            }
                        }
                    });
                }
