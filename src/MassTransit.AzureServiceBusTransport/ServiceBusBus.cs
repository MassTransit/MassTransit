...
namespace MassTransit.AzureServiceBusTransport
{
        ...
        IBusControl
    {
        ...
        public ServiceBusBus(Uri address, IConsumePipe consumePipe, ISendEndpointProvider sendEndpointProvider,
            IEnumerable<IReceiveEndpoint> receiveEndpoints, IServiceBusHost[] hosts, IPublishEndpoint publishEndpoint)
        {
            _address = address;
        ...
        class Handle :
            BusHandle
        {
            readonly ServiceBusBus _bus;
            readonly ReceiveEndpointHandle[] _receiveEndpoints;

            public Handle(ServiceBusBus bus, ReceiveEndpointHandle[] receiveEndpoints)
            {
                _bus = bus;
            ...
            public void Dispose()
            {
                ...
                Task.WaitAll(_bus._hosts.Select(x => x.Close()).ToArray());
            }
            ...
            async Task BusHandle.Stop(CancellationToken cancellationToken)
            {
                ...
                await Task.WhenAll(_bus._hosts.Select(x => x.Close()));
            }
        }
    }
}