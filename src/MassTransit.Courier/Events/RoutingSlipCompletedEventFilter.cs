...
namespace MassTransit.Courier.Events
{
    ...
    public class RoutingSlipCompletedEventFilter :
        IRoutingSlipEventFilter
    {
        T IRoutingSlipEventFilter.Filter<T>(T message, RoutingSlipEventContents contents)
        {
            var source = message as RoutingSlipCompleted;
            if (source == null)
                return message;

            RoutingSlipCompleted result = new RoutingSlipCompletedMessage(
                source.TrackingNumber,
                source.Timestamp, source.Duration,
