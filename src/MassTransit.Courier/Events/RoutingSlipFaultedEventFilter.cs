...
namespace MassTransit.Courier.Events
{
    ...
    public class RoutingSlipFaultedEventFilter :
        IRoutingSlipEventFilter
    {
        T IRoutingSlipEventFilter.Filter<T>(T message, RoutingSlipEventContents contents)
        {
            var source = message as RoutingSlipFaulted;
            if (source == null)
                return message;

            RoutingSlipFaulted result = new RoutingSlipFaultedMessage(
                source.TrackingNumber,
                source.Timestamp,
                source.Duration,
                source.ActivityExceptions,
