...
namespace MassTransit.Courier.Events
{
    ...
    public class RoutingSlipActivityCompensatedEventFilter :
        IRoutingSlipEventFilter
    {
        T IRoutingSlipEventFilter.Filter<T>(T message, RoutingSlipEventContents contents)
        {
            var source = message as RoutingSlipActivityCompensated;
            if (source == null)
                return message;

            RoutingSlipActivityCompensated result = new RoutingSlipActivityCompensatedMessage(
                source.Host,
