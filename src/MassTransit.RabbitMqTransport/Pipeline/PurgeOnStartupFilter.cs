...
namespace MassTransit.RabbitMqTransport.Pipeline
{
    ...
    {
        static readonly ILogger _logger = Logger.Get<PurgeOnStartupFilter>();
        ...
        async Task PurgeIfRequested(ModelContext context, QueueDeclareOk queueOk, string queueName)
        {
            ...
            {
                _logger.LogDebug("Purging {0} messages from queue {1}", queueOk.MessageCount, queueName);

                var purgedMessageCount = await context.QueuePurge(queueName).ConfigureAwait(false);

                _logger.LogDebug("Purged {0} messages from queue {1}", purgedMessageCount, queueName);

                _queueAlreadyPurged = true;
            }
            else
            {
                _logger.LogDebug("Queue {0} already purged at startup, skipping", queueName);
            }
        }
    }
}
