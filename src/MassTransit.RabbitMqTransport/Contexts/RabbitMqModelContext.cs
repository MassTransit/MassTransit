...
namespace MassTransit.RabbitMqTransport.Contexts
{
    ...
    public class RabbitMqModelContext :
        ...
        IAsyncDisposable
    {
        static readonly ILogger _logger = Logger.Get<RabbitMqModelContext>();
        ...
        {
            _logger.LogDebug("Closing model: {0} / {1}", _model.ChannelNumber, _connectionContext.Description);
            ...
            try
            {
                if (_connectionContext.PublisherConfirmation && _model.IsOpen && _published.Count > 0)
                {
                    ...
                    do
                    {
                        ...
                        if (timedOut)
                            _logger.LogWarning("Timeout waiting for pending confirms: {0}", _connectionContext.Description);
                        else
                            _logger.LogDebug("Pending confirms complete: {0}", _connectionContext.Description);
                    }
                    while (timedOut);
                }
            }
            ...
            {
                _logger.LogError("Fault waiting for confirms", ex);
            }
        ...
        {
            _logger.LogDebug("BasicReturn: {0}-{1} {2}", args.ReplyCode, args.ReplyText, args.BasicProperties.MessageId);
