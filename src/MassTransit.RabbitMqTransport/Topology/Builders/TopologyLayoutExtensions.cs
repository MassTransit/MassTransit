...
namespace MassTransit.RabbitMqTransport.Topology.Builders
{
    ...
    public static class TopologyLayoutExtensions
    {
        ...
        public static void LogResult(this BrokerTopology layout)
        {
            ...
            {
                _logger.LogInformation("ExchangeName: {0}, type: {1}, durable: {2}, auto-delete: {3}", exchange.ExchangeName, exchange.ExchangeType, exchange.Durable, exchange.AutoDelete);
            }

            foreach (var binding in layout.ExchangeBindings)
            {
                _logger.LogInformation("ExchangeName Binding: source {0}, destination: {1}, routingKey: {2}", binding.Source.ExchangeName, binding.Destination.ExchangeName,
                    binding.RoutingKey);
            }
        }
    }
