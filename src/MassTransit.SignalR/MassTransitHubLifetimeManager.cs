...
namespace MassTransit.SignalR
{
    ...
    public class MassTransitHubLifetimeManager<THub> : BaseMassTransitHubLifetimeManager<THub>
        where THub : Hub
    {
        static readonly ILogger _logger = Logger.Get<MassTransitHubLifetimeManager<THub>>();
        ...
        {
            _logger.LogInformation("Publishing All<THub> message to MassTransit.");
        ...
        {
            _logger.LogInformation("Publishing All<THub> message to MassTransit, with exceptions.");
            return _publishEndpoint.Publish<All<THub>>(
                new
                {
                    Messages = _protocols.ToProtocolDictionary(methodName, args),
        ...
        public override Task SendConnectionAsync(string connectionId, string methodName, object[] args, CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("Publishing Connection<THub> message to MassTransit.");
            return _publishEndpoint.Publish<Connection<THub>>(new
                {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (connectionIds.Count > 0)
            {
                ...
                _logger.LogInformation("Publishing multiple Connection<THub> messages to MassTransit.");
        ...
        public override Task SendGroupAsync(string groupName, string methodName, object[] args, CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("Publishing Group<THub> message to MassTransit.");
            return _publishEndpoint.Publish<Group<THub>>(new
                {
                    GroupName = groupName,
                    Messages = _protocols.ToProtocolDictionary(methodName, args)
                },
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("Publishing Group<THub> message to MassTransit, with exceptions.");
            ...
            if (groupNames.Count > 0)
            {
                ...
                foreach (var groupName in groupNames)
                {
                    if (!string.IsNullOrEmpty(groupName))
                        publishTasks.Add(_publishEndpoint.Publish<Group<THub>>(new
                        {
                            GroupName = groupName,
                            Messages = protocolDictionary
                        }, cancellationToken));
                ...
                _logger.LogInformation("Publishing multiple Group<THub> messages to MassTransit.");
                return Task.WhenAll(publishTasks);
            }
        ...
        {
            _logger.LogInformation("Publishing User<THub> message to MassTransit.");
            return _publishEndpoint.Publish<User<THub>>(new
            {
                UserId = userId,
                Messages = _protocols.ToProtocolDictionary(methodName, args)
            }, cancellationToken);
        ...
        public override Task SendUsersAsync(IReadOnlyList<string> userIds, string methodName, object[] args, CancellationToken cancellationToken = default)
        {
            ...
            if (userIds.Count > 0)
            {
                ...
                foreach (var userId in userIds)
                    publishTasks.Add(_publishEndpoint.Publish<User<THub>>(new
                    {
                        UserId = userId,
                        Messages = protocolDictionary
                    }, cancellationToken));

                _logger.LogInformation("Publishing multiple User<THub> messages to MassTransit.");
                return Task.WhenAll(publishTasks);
            }
        ...
        public override async Task AddToGroupAsync(string connectionId, string groupName, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                _logger.LogInformation("Publishing add GroupManagement<THub> message to MassTransit.");
                ...
                _logger.LogInformation($"Request Received for add GroupManagement<THub> from {ack.Message.ServerName}.");
            }
            catch (RequestTimeoutException e)
            {
                // That's okay, just log and swallow
                _logger.LogWarning("GroupManagement<THub> add ack timed out.", e);
            }
        }
        ...
        public override async Task RemoveFromGroupAsync(string connectionId, string groupName, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                _logger.LogInformation("Publishing remove GroupManagement<THub> message to MassTransit.");
                RequestHandle<GroupManagement<THub>> request =
                    _groupManagementRequestClient.Create(new
                        {
                            ConnectionId = connectionId,
                ...
                Response<Ack<THub>> ack = await request.GetResponse<Ack<THub>>().ConfigureAwait(false);
                _logger.LogInformation($"Request Received for remove GroupManagement<THub> from {ack.Message.ServerName}.");
            }
            catch (RequestTimeoutException e)
            {
                // That's okay, just log and swallow
                _logger.LogWarning("GroupManagement<THub> remove ack timed out.", e);
            }
        }
