...
namespace MassTransit.Azure.ServiceBus.Core.Contexts
{
    ...
    public class QueueClientContext :
        ...
        IAsyncDisposable
    {
        static readonly ILogger _logger = Logger.Get<QueueClientContext>();
        ...
        {
            _logger.LogDebug("Closing client: {0}", InputAddress);
            ...
            try
            {
                ...
                _logger.LogDebug("Closed client: {0}", InputAddress);
            }
            catch (Exception exception)
            {
                _logger.LogWarning($"Exception closing the client: {InputAddress}", exception);
            }
        }
