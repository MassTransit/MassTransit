        ...
        async Task Receiver()
        {
            while (!IsStopping)
            {
                try
                {
                    ...
                    {
                        _logger.LogDebug("Connecting receive transport: {0}", _receiveEndpointContext.InputAddress);
                        ...
                        {
                            _logger.LogError($"ReceiveTransport Faulted: {_receiveEndpointContext.InputAddress}", ex);
