...
namespace MassTransit.ServiceBus.Pipeline
{
	...
	public static class MessageRouterExtensions
	{
		...
		{
			return router.Connect(new MessageSink<TMessage>(message => consumer.Accept(message) ? consumer : Consumes<TMessage>.Null));
		}

		public static Func<bool> SubscribeTo<TMessage>(this Consumes<TMessage>.All consumer, MessageRouter<TMessage> router)
			where TMessage : class
		{
			return router.Connect(new MessageSink<TMessage>(message => consumer));
		}
	}
}